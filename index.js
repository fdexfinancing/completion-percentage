...
class PercentageDisplay {
  calculate(mainStructure, data) {
    ...
    mainStructure.survsAreaList.forEach(area => {
      ...
      area.structure.all.forEach(structure => {
        ...
        areaPercentual += this.calculateQuestion(structure, data);
      });
  ...
  calculateQuestion(structure, data) {
      ...
      if (questionArr.length > 1) {
        ...
        if (rule === ROW_TEMPLATE_RULE) {
          ...
          for(let i = 1; i <= structure.parent.min_required; i++) {
            ...
            if (this.isValidQuestion(rule, questionData, question)) {
        ...
        if (data[questionArr[0]]) {
          if (this.isValidQuestion(rule, data[questionArr[0]], question)) {
            currentPercentage += questionWeigth;
          }
        }
      }
    ...
    return this.showPercentage(structure.parent.rule, currentPercentage);
  }

  isValidQuestion(rule, data, question) {
  ...
  showPercentage(rule, percentage) {
...
module.exports = new PercentageDisplay();
