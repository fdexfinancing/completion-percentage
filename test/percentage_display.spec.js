const PercentageDisplay = require('../index');
...
test('Should get a percentage display for DefaultRule', () => {
    ...
    expect(PercentageDisplay.calculateQuestion(defaultRuleStruct, data)).toEqual(1);
});
...
test('Should get 0% display for DefaultRule', () => {
    ...
    expect(PercentageDisplay.calculateQuestion(defaultRuleStruct, data)).toEqual(0);
});
...
test('Should get percentage for RowTemplateRule', () => {
  ...
  expect(PercentageDisplay.calculateQuestion(rowTemplateStruct, data)).toEqual(1);
});
...
test('Should get granular percentage for RowTemplateRule', () => {
  ...
  expect(PercentageDisplay.calculateQuestion(rowTemplateStruct, data)).toEqual(0.75);
});
...
"economic_activity4_explanation":'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.'},"other_situation":"YTeste"};
  ...
  expect(PercentageDisplay.calculateQuestion(dropdownRuleStruct, data)).toEqual(1);
});
...
test('Should get 75 percent of DropdownRule', () => {
  ...
  expect(PercentageDisplay.calculateQuestion(dropdownRuleStruct, data)).toEqual(0.5);
});
...
test('Should get hundred percent of UploadRule', () => {
  ...
  expect(PercentageDisplay.calculateQuestion(uploadRuleStruct, data)).toEqual(1);
});
...
test('Should get 50% percent of UploadRule', () => {
  ...
  expect(PercentageDisplay.calculateQuestion(uploadRuleStruct, data)).toEqual(0.50);
});

test('should return real percentage', () => {
  let questions = (1 / 5);
  ...
  let result = questions * (a + b + c + d + e) * 100;

  expect(result.toFixed(2)).toEqual("60.00");
});
...
test('Should calculte for all areas', () => {
  ...
  expect(PercentageDisplay.calculate(allSurvsStruct.result, data)).toEqual(expectedResult);
});
